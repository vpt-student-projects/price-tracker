...
namespace PriceTracker.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public async Task<IActionResult> RefreshPrice(int id)
        {
            ...
            try
            {
                _logger.LogInformation("Начинаем парсинг цены для {ProductName} ({Url})", product.Name, product.Url);
                ...
                if (price.HasValue)
                {
                    ...
                    _logger.LogInformation("Цена успешно обновлена: {ProductName} - {Price}₽", product.Name, price.Value);
                ...
                {
                    _logger.LogWarning("Цена не найдена на странице: {Url}", product.Url);
                    return Ok(new
                    {
                        product.Id,
                        message = "Цена не найдена на странице. Проверьте ссылку или структуру сайта."
                    });
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Ошибка при обновлении цены для {Url}", product.Url);
